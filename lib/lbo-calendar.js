...
// LBO renders months as "Oct<span>ober</span>" (short form on phones), which
// becomes "Oct ober" once tags are spaced apart. Glue such pieces back together.
function joinSplitMonths(text) {
  return text.replace(/\b(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec) ([a-z]+)\b/g, (all, a, b) => MONTHS.includes(a + b) ? a + b : all);
}

// Opera, ballet, dance and circus listings that LBO files under musicals.
function looksNonMusical(name) {
  return /ballet|and opera\b|national opera|opera house|\bdance company|tango|flamenco|circus|cirque|in concert|improvised|magic mike|panto/i.test(name);
}
...
export function parseMonthCalendar(html, year, month) {
  ...
  const text = joinSplitMonths($("body").text().replace(/\s+/g, " "));
  let venue = "";
...
async function getMonth(fetchText, slug, year, month) {
  ...
  const data = { ...parseMonthCalendar(await fetchText(url), year, month), url };
...
export async function findMissingPerformances({ dates, isFound, fetchText }) {
  const result = { shows: [], performances: {} };
  let list;
  try { list = await getMusicalsList(fetchText); } catch (err) {
    console.error("LBO musicals list failed:", err?.message || err);
    return result;
  }
  const missing = list.filter(s => !isFound(s.name) && !looksNonMusical(s.name)).slice(0, MAX_FALLBACK_SHOWS);
